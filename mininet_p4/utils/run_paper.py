...
class ExerciseRunner:
    ...
    def generate_data(self):
        ...
        generate_dataset = pd.read_csv(generate_data_path)
    ...
    def visit_data(self):
        ...
        visit_dataset = pd.read_csv(visit_data_path)
        for index, row in visit_dataset.iterrows():
            ...
            target_ip = "10.0."+target+"."+target
            coordinate = "(%.4f,%.4f)"%(row["x"],row["y"])
            cmd = "./client.py -i %s -a \"visit\" -c \"%s\""% (target_ip, coordinate)
            h = self.net.get(host_name)
            h.cmd(cmd)
