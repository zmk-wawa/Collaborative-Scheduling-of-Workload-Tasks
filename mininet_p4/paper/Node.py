...
class Node:
    def __init__(self, ip, port, host_id):
        self.filenameList = []
        self.ip = ip
    ...
    def start(self):
        ...
        time.sleep(self.host_id)
        self.sendJoinRequest("10.0.1.1", 13777)
        time.sleep(20)
        # In case of connecting to other clients
        # while True:
            # print("Listening to other clients")   
            # self.asAClientThread()
        threading.Thread(target=self.generateIndex, args=()).start()
        time.sleep(150)
        print(self.filenameList)
        threading.Thread(target=self.visitData(), args=()).start()
        while(True):
            time.sleep(20)
    ...
    def generateIndex(self):
        ...
        for index, row in generate_dataset.iterrows():
            src = int(row["host_id"])
            if src == self.host_id:
                filename = "(%.4f,%.4f)"%(row["x"],row["y"])
                fileID = getHash(filename)
                start = round(time.time()*1000)
                recvIPport = self.getSuccessor(self.succ, fileID)
                end = round(time.time()*1000)
                thisTime = end-start
                self.generateUseTime += thisTime
                self.useTime += thisTime
                self.generateCounter += 1
                self.counter += 1
                print("====== total ====== time:%d, counter:%d"%(self.useTime, self.counter))
                print("Total generate time:%d, this generate time:%d, generate counter:%d"%(self.generateUseTime, thisTime, self.generateCounter))
                # print(recvIPport)
                self.uploadFile(filename, recvIPport, True)

    def visitData(self):
        visit_data_path = "/home/p4/tutorials/exercises/paper/dataset/visit_p50_dataset.csv"
        visit_dataset = pandas.read_csv(visit_data_path)
        for index, row in visit_dataset.iterrows():
            src = int(row["host_id"])
            if src == self.host_id:
                filename = "(%.4f,%.4f)"%(row["x"],row["y"])
                fileID = getHash(filename)
                ...
                if fileData == b"NotFound":
                    self.visitFail +=1
                self.visitUseTime += thisTime
                self.useTime += thisTime
                self.visitCounter += 1
                self.counter += 1
                print("====== total ====== time:%d, counter:%d"%(self.useTime, self.counter))
                print("Total visit time:%d, this visit time:%d, visit counter:%d, fail:%d"%(self.visitUseTime, thisTime, self.visitCounter, self.visitFail))
                # print(recvIPport)
